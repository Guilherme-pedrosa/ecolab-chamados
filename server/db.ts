...
import { InsertUser, users, chamados, InsertChamado, evolucoes, InsertEvolucao } from "../drizzle/schema";
...
export async function getAllChamados() {
  ...
  return await db.select().from(chamados).orderBy(chamados.dataOS);
}
...
// Evoluções queries
export async function createEvolucao(evolucao: InsertEvolucao) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  const result = await db.insert(evolucoes).values(evolucao);
  return result;
}

export async function getEvolucoesByChamadoId(chamadoId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  return await db.select().from(evolucoes).where(eq(evolucoes.chamadoId, chamadoId)).orderBy(evolucoes.createdAt);
}
